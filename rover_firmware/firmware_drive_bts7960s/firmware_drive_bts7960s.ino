...
// PCA9685 for motor PWM (BTS7960 RPWM/LPWM) + steering servos
// Address 0x40 by default
Adafruit_PWMServoDriver pwm = Adafruit_PWMServoDriver(0x40);

// =========================
// BTS7960 MOTOR DRIVER WIRING
// One BTS7960 per wheel
//
// For each wheel i:
//   - RPWM is driven from PCA9685 channel MOTOR_RPWM_CH[i]
//   - LPWM is driven from PCA9685 channel MOTOR_LPWM_CH[i]
//   - L_EN and R_EN on the BTS7960 breakout are tied together
//     and connected to Arduino pin MOTOR_EN_PINS[i], held HIGH.
// =========================

// Arduino EN pins (to BTS7960 L_EN and R_EN, tied together per board)
const uint8_t MOTOR_EN_PINS[NUM_WHEELS]  = {23, 25, 27, 29, 31, 33};

// PCA9685 channels for BTS7960 RPWM (forward) and LPWM (reverse)
// 6 wheels → 12 channels (0..11).
const uint8_t MOTOR_RPWM_CH[NUM_WHEELS] = { 0,  2,  4,  6,  8, 10};
const uint8_t MOTOR_LPWM_CH[NUM_WHEELS] = { 1,  3,  5,  7,  9, 11};

// PCA9685 channels for steering servos (remaining 4 channels)
const uint8_t STEER_PWM_CH[NUM_STEER] = {12, 13, 14, 15};

// Encoder pins (A=interrupt, B=digital) – unchanged
...
void setupMotors() {
  for (int i = 0; i < NUM_WHEELS; i++) {
    // BTS7960: only EN pin is on Arduino; RPWM/LPWM are on the PCA9685.
    pinMode(MOTOR_EN_PINS[i], OUTPUT);
    digitalWrite(MOTOR_EN_PINS[i], HIGH);  // enable all drivers by default
  }

  pwm.begin();
  // 1 kHz is OK for BTS7960; adjust if you prefer a different PWM frequency.
  pwm.setPWMFreq(1000);
}

void setMotorSpeed(int idx, float cmd) {
  // BTS7960: one driver per wheel, driven by two PWM signals (RPWM/LPWM) from PCA9685.
  // cmd in range [-1.0, 1.0] (or any value; it will be clipped).
  //   cmd > 0: forward  -> RPWM = |cmd|, LPWM = 0
  //   cmd < 0: reverse  -> RPWM = 0,       LPWM = |cmd|
  //   cmd = 0: brake/idle -> both 0

  // 1. Make sure the driver for this wheel is enabled
  digitalWrite(MOTOR_EN_PINS[idx], HIGH);

  // 2. Compute magnitude and clip
  float mag = fabs(cmd);
  if (mag > 1.0f) mag = 1.0f;
  uint16_t pwm_val = (uint16_t)(mag * 4095.0f);

  uint8_t ch_r = MOTOR_RPWM_CH[idx];
  uint8_t ch_l = MOTOR_LPWM_CH[idx];

  if (pwm_val == 0) {
    // Stop / brake: both sides off
    pwm.setPWM(ch_r, 0, 0);
    pwm.setPWM(ch_l, 0, 0);
    return;
  }
  ...
  if (cmd > 0.0f) {
    // Forward: RPWM active, LPWM low
    pwm.setPWM(ch_r, 0, pwm_val);
    pwm.setPWM(ch_l, 0, 0);
  } else {
    // Reverse: LPWM active, RPWM low
    pwm.setPWM(ch_r, 0, 0);
    pwm.setPWM(ch_l, 0, pwm_val);
  }
}
